// prod-smoke.checks.mjs — THIS project's production watchdog assertions (the engine is
// scripts/prod-smoke.mjs, byte-identical to the dobby-foundation template's). Moved here verbatim from the
// engine when it became the template's copy (plugin-audit-and-extraction S3); the reasoning behind each
// check is in its `why` and the comments below. Never make a red smoke pass by weakening a check.

import { JS_MEDIA_TYPES } from './prod-smoke.mjs';

export const BASE = 'https://miyagisanchez.com';
