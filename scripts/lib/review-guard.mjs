...
// bypass it — and strict only about "is there a review in here at all".
//
...
export function decideSecurityPass({ files = [], body = '', securityPaths = [] }) {
  ...
  if (/\brisk(?:\s*tier)?\s*[:=]?\s*\**\s*high\b/i.test(body))
 ...
 */
export function postReviewStatus({ pr, repo, state, lens, description }, deps = {}) {
...
/** Re-review convergence (D8): a prior comment for the same lens means Important findings only. */
export function isReReview(commentBodies = [], lens = null) {
  const title =
    lens === 'security' ? /^### 🔐 Cross-agent review — security lens/m : /^### 🔎 Cross-agent review \(/m;
  return commentBodies.some((b) => title.test(String(b)));
}

export const RE_REVIEW_NOTE =
  '\n\n## RE-REVIEW\nA previous pass already reviewed this PR and the author has pushed changes since. Report **Blocking / Important findings only** — no Should-fix, no nits, and do not repeat a finding the author has already fixed or answered on the PR.\n';
