...
- ✅ **Seller-portal depth pass (P2·E)** — every seller route now acknowledges navigation with a content-shaped skeleton; listing deletion has a real 10-second `Deshacer`; order bulk status changes show a per-order current → proposed preview and explicit refusal reasons before apply; guidance is factual es-MX with no invented performance claims; and mobile selection/row actions use thumb-safe targets plus one `Más` sheet. Backend PR [#161](https://github.com/danybgoode/medusa-bonsai-backend/pull/161) merged before frontend PR [#388](https://github.com/danybgoode/miyagisanchezcommerce/pull/388). *(Authenticated mixed-order smoke remains owed to Daniel because the local test-seller fixture is not provisioned.)*
...
- ✅ **UCP buyer-side shipping exposure** — agents now discover real seller-authoritative shipping/pickup
  choices through the existing UCP checkout session and MCP `get_checkout_options`, then submit only a
  returned opaque selection id to `create_checkout`. Immediately before its cart write, the server
  refreshes the delivery catalog/rate and derives carrier, service and money from that current backend
  object, so stale, cross-address or invented ids cannot alter a charge. No new flag, MCP tool, database,
  provider or payment calculation; arranged/rental/digital and legacy calls stay unchanged. Production
  Cloud Run revision `miyagi-web-00106-plc` is live. *(The real test-mode rate→hosted-total→order and
  structured-pickup checks need a safe published fixture; none exists in the public catalog today.)*
...
- ✅ **The Living Shop — a storefront that feels inhabited** — a merchant's own shop is no longer a branded product grid. Its homepage is a **Wall**: a chronological narrative where a note, a featured product, a story around a collection and an invitation to an event all live together, pinned and scheduled by the seller. Commerce lives *inside* it — a product card carries the current price and links to the real PDP, and a reference whose object is gone simply disappears rather than becoming a stale buyable price, because the Wall stores a reference and **never a copy**. The shop gets a **closed set of seven sections** (Muro · Tienda · Colecciones · Eventos · Acerca · Preguntas · Políticas) with one nav across marketplace, subdomain and custom domain, and **three theme modes**: Default, a finished **Retro Social**, and **Custom** — substantial no-code control through a typed schema with *no* field that can carry CSS, HTML, JavaScript or a font URL, asserted over the schema by spec. The five shipped legacy presets keep working with **no backfill and no visual reset**. A seller authors all of it from one studio (`/shop/manage/tienda`) with a live preview that renders the real public shop, and their agent can do the same through four MCP Wall tools plus a `presentation` block in Storefront-as-Code — sharing the *same* validator as the human API, so an agent can never reach a laxer rule than a person. Scheduling runs with **no cron**: an entry becomes public because its instant has passed, decided at read time. *(Owed: Daniel's authed seller walkthrough — `pk_test_` locally vs `pk_live_` in production; the authed cross-shop refusal; and one real event, since `marketplace_events` holds zero rows platform-wide.)*
- ✅ **`/us` is an open marketplace — a US merchant sells in USD into their own Stripe account** — the same promise Mexico already makes, no commission, in a second country. A merchant signing up from `/us` gets a shop whose operating market **is** `us` (before this the signup carried no market at all, so they silently got a *Mexican* shop pricing in pesos); a buyer browses `/us` in English and checks out in USD through a Stripe Connect **direct charge** on the seller's own connected account — no `transfer_data`, no application fee, refunds without `reverse_transfer`, and the platform account cannot even retrieve the charge, which is what "direct" means. Delivery is **arranged or manual-carrier**: the US has no carrier integration, so it is never offered a rate it cannot honour — the seller ships with their own carrier and enters real tracking, which is *required* before an order can be marked shipped. Money is currency-correct end to end: every profit aggregation key includes currency (USD and MXN are shown separately and **never summed** — before this they were added together as though cents were a unit), and orders, receipts and refunds render in their own locale (a \$12.50 US receipt was rendering as `USD 13`). Agent surfaces answer `market=us` with the US catalog; US listings deliberately carry no per-listing checkout URL, because the legacy ones are Mexican rails that would refuse them. **Proven in Stripe test mode, 21/21 checks, driven through the shipped code** → **[Epic ›](07-agentic-and-federated-commerce/us-marketplace/)**. *(Owed: the first real USD charge — Daniel's, by design; ~550 strings of seller-portal body copy still es-MX; processor-fee ledger events, absent for both markets; one US carrier, deferred at its evidence gate.)*
- ✅ **Clerk JWTs are actually verified** — `/store/sellers/me/**` and `/store/buyer/me/**` decoded a bearer token and trusted its `sub` **without checking the signature**, on the strength of a code comment claiming edge middleware did it. No such middleware existed anywhere in the repo. A forged `alg:none` token was accepted as identity on 38 routes — products, orders, coupons, payouts, Stripe onboarding. Confirmed live, closed, and verified closed after deploy; coverage is now derived from the route tree so a new authenticated route that forgets it goes red.
- ✅ **Country markets — `/` selects; `/mx` and `/us` sell** — market, locale, checkout region, and marketplace publication are **four separate concepts** instead of one implicit "Mexico". `/` is the master-brand selector, while `/mx` and `/us` are live marketplaces with market-scoped catalog, currency and checkout rails. A shop **does not need marketplace admission**: owned-shop reads resolve by seller ownership + publish state, so custom domains, subdomains and embeds keep working with zero marketplace membership and never carry a country prefix. Locale is presentation only — `requireMarket('es-MX')` throws and says "that looks like a LOCALE". **The selector is bilingual since 2026-08-19**: `/` (Spanish) and `/en` (English) are two static documents, not one page sniffing `Accept-Language` — Cloudflare fronts Cloud Run and does not vary its cache key on arbitrary headers, so a sniffing page would serve one visitor's language to everyone behind that cache entry. A client island hops once toward the document the visitor can read; the ES/EN switcher stores a choice that outranks the browser from then on. Agent surfaces take an optional `market`, echo `market_code`, and return the requested market's catalog without falling through to another country. Medusa remains authoritative for Region and Sales-Channel publication. The invitation-era `/us` contract was superseded by the US marketplace epic; operator recruiting now lives at `/us/operators`.
...
- 📋 **Sell on your own shop without listing in the marketplace** *(scaffolded)* — country markets made "a shop does not need marketplace admission" true for **reads**; it is not yet true for **money**, because a product in no Medusa Sales Channel cannot be bought. This adds a second channel per market — an **operating** channel that carries buyability, leaving the marketplace channel to mean publication and nothing else — so a merchant can say "sell this on my shop, don't list it in Mexico". Sequenced backfill-first (create → protect → backfill → only then make membership load-bearing), because a publishable key scoped to a half-filled channel serves a partial catalog that looks healthy. See [07 · Agentic & Federated Commerce › Owned-shop operating channel](07-agentic-and-federated-commerce/owned-shop-operating-channel/).
...
- 🏗️ **Miyagi Operators (US) — the Promotor program, told in English** — `/us/operators` recruits the
  same field operator `/vende/promotor` recruits in Mexico: walk into a local business, open its store
  before you leave, take payment in person, keep the commission. Rebuilt on the shared brand landing
  shell (hero + AI prompt block + a **Miyagi vs Shopify vs Amazon** comparison table, US seller rates
  verified and date-stamped) with both locales' copy written from scratch. The application is the same
  five fields as the Mexican one — the retired three-shop "founding proof" dossier, its disqualification
  gates and its 90-day consent are gone, along with the disclaimer-first voice they came with;
  `operator_details` v2 plus the CHECK constraint that was its real contract, **applied and verified
  live 2026-08-17**. **No dollar figures for our own SKUs**: US pricing does not exist in code, so rates
  are set on approval rather than invented. Storefront PR #386, on top of #342 (`3aba592`), #343
  (`1709226`), #350 (`5d4df0c`) and #384. `partners.recruiting_v3_enabled` has a Golden definition
  serving version 2 default-ON from the owner-visible `miyagisanchez` catalog, and the scoped catalog
  now has its own durable snapshot lane preventing cold-start false 404s, while the older 43-decision
  catalog remains untouched. Closeout waits only on Daniel's authenticated walkthrough and one real
  submission. See [epic ›](08-growth-and-promotions/operator-program-copy/) and
  [recruiting v3 ›](08-growth-and-promotions/miyagi-partners-recruiting-v3/).
...
- ✅ **Seller-acquisition landing pages** — a supply-side recruitment funnel that pitches *selling on Miyagi* and converts on `/sign-up`. **One landing per market since 2026-08-19**: the whole Spanish family moved under `/mx/vende` (308 from `/vende/*`, query preserved, so printed flyers and paid traffic keep working) and the US landing became a real route at `/us/sell` — before this it had no URL at all, rendering only as the signed-out branch of `/sell` when the request happened to carry `?market=us`, so `/sell` served the Mexican hero, in Spanish, to a US merchant. `SELLER_LANDING_PATHS` is the one place that knows where a market recruits, swept by a source guard because a compatibility redirect makes a stale link invisible to every route test. The family: an anchor `/mx/vende` page + a persona router with tailored es-MX pitches (`/vende/creadores`, `/vende/negocios`, `/vende/servicios`) and a World-Cup wedge (`/vende/mundial`), per-persona SEO/OG, Clarity + UTM attribution, and A/B hooks. **Content overhaul (v2, 2026-06-26):** real intentional es-MX copy (distrust framing + internal jargon killed; correct accents/ñ/¿¡, CI-guarded), a directive copy-paste "ask your agent" prompt, a truthful **Miyagi vs Mercado Libre vs Shopify benchmark** (0%-platform-commission framing, sourced + date-stamped), a **UCP/MCP AI-channel** value-prop section, and a **mobile-responsive sweep** (no overflow at 360/390/414px). Frontend-only.
- ✅ **Tiendas Fundadoras acquisition** — a focused public invitation for the first **25 founding shops** at `/vende/fundadoras`: one promise (hands-on setup + a private preview before publication), one attributed application that writes into the **canonical activation relationship** (not a second leads table, no Medusa seller created by applying). Contact consent and preview-permission are **separate** affirmative choices in an append-only ledger; the funnel is measured with **PII-free** opaque-subject events. Capacity-aware closed/open/full state read from canonical rows (25-seat, server-enforced, never a client counter). Shipped PR #306, migration applied + prod smoke passed; **LIVE** behind `growth.founding_merchants_enabled` (enablement, **ON** 2026-07-24 — 0 real applicants yet).
...
- **2026-08-18 — Living Shop SHIPPED (7 sprints, LOW throughout — no money path).** The own-shop
  ...
  theme modes including a schema-driven Custom with no code escape hatch. Built in one orchestrated
  run behind a locking pass that **corrected three scaffolded premises before any code was written** —
  `marketplace_events` holds zero rows platform-wide, `middleware.ts` already passes every non-root
  path through on both owned hosts (so no middleware change was needed, disproved as the sprint doc
  asked), and only three shops carry a legacy preset, which turned a data migration into a read-time
  mapping and removed the epic's only real chance of regressing a live storefront. The migration was
  **applied live and every CHECK made to actually refuse** what it forbids inside a transaction that
  then aborted, so the table came back empty. **Five real defects in the build were caught by the
  existing deterministic gate, not by review** — including `getShop()` returning the Medusa seller
  whose id is not the `marketplace_shops.id` the foreign key points at (correct-looking code that
  would have matched zero rows silently), and section routes that made a URL-prefix decision instead
  of a market decision, which would have rendered a US merchant's catalog under `/mx`. Widening the
  buyer-copy scan to the owned-host root routes — which had **never** been scanned — surfaced a live
  bilingual leak on `/acerca`. Two of the run's own new specs **survived their red-mutation check**
  and had to be rewritten: one asserted a directory constant against itself, the other used a
  substring match a rename satisfied. Durable levers promoted to LEARNINGS. *(Owed: Daniel's authed
  seller walkthrough; the authed cross-shop refusal; one real event.)* See
  [07 · Agentic & Federated Commerce › Living Shop](07-agentic-and-federated-commerce/living-shop-social-storefront/).
...
- **2026-07-24 — Tiendas Fundadoras acquisition SHIPPED + LIVE (HIGH; area 08 ·
  ...
  focused public invitation for the first **25 founding shops** at
  ...
  **server-side at the write**, so a stale page or a direct API call can't seat past a closed/full cohort; a
  ...
  request-scoped flag. Go-live executed same day (Daniel authorized once CI was green; site is pre-launch):
  migration applied + verified live → merged + deployed (`miyagi-web-00025-t68`) → prod smoke passed (flag-OFF
  closed/404 → flag-ON open → disposable application wrote one canonical relationship with UTM + dropped-invalid
