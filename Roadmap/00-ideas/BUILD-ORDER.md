<!-- GENERATED FILE — do not edit by hand.
     Regenerate:  node scripts/build-order.mjs
     Source of truth: seed frontmatter (status/priority/epic) + epic & sprint status lines,
     via the same projection the Notion sync reads (scripts/roadmap-to-notion.mjs --extract). -->

# Build order — generated status board

> **Generated 2026-06-14 from frontmatter — do not hand-edit.** To change what this shows, edit the
> seed/epic/sprint docs (status lives there), then run `node scripts/build-order.mjs`.
> Status & ordering single-source-of-truth = **seed frontmatter**; this board and the Notion
> "Marketplace Roadmap" DB are both *derived views* of it.

## 🏗️ Building now (3)

- [PDP redesign: "decide, then act"](../../01-discovery-and-shopping/pdp-redesign/README.md) — 01 Discovery · 11/18 stories
- [Buyer Telegram channel + Buyer preference center](../../05-trust-offers-and-messaging/buyer-notifications/README.md) — 05 Trust/Offers/Messaging · 2 sprints · risk: High · Wave 2
- [Granular Multi-Channel Notifications (Email + Telegram)](../../05-trust-offers-and-messaging/granular-notifications/README.md) — 05 Trust/Offers/Messaging · 7/7 stories · risk: High · Wave 2

## 📋 Ready to build (scaffolded, not started) (4)

- [Delivery & Manual-Money Polish](../../02-checkout-and-payments/delivery-money-polish/README.md) — 02 Checkout & Payments · 5/8 stories · risk: High · Wave 3
- [Trust & Messaging Polish](../../05-trust-offers-and-messaging/trust-messaging-polish/README.md) — 05 Trust/Offers/Messaging · 2 sprints · risk: Low · Wave 3
- [Cross-channel Storefront Trust Parity](../../07-agentic-and-federated-commerce/cross-channel-trust-parity/README.md) — 07 Agentic/Federated · 1 sprints · risk: Low · Wave 3
- [PWA Liquid-Glass Nav Polish](../../09-platform-infra/pwa-liquid-glass-nav-polish/README.md) — 09 Platform-infra · 0/6 stories · risk: Low

## ✅ Shipped (38)

- [Discovery Polish](../../01-discovery-and-shopping/discovery-polish/README.md) — 01 Discovery · 7/7 stories
- [Homepage Polish — Dirección B «Catálogo limpio»](../../01-discovery-and-shopping/homepage-polish-b/README.md) — 01 Discovery · 1/11 stories · risk: Low
- [Neighborhood Pulse — online community feed](../../01-discovery-and-shopping/neighborhood-pulse/README.md) — 01 Discovery · 7/7 stories · risk: Low
- [PDP interactive image gallery ✅ COMPLETE](../../01-discovery-and-shopping/pdp-image-gallery/README.md) — 01 Discovery · 2/2 stories · risk: Low
- [Checkout & Manual-Payment State Hardening](../../02-checkout-and-payments/checkout-state-hardening/README.md) — 02 Checkout & Payments · 9/9 stories · risk: High · Wave 2
- [Agent-native setup (Onboarding 0) — ✅ COMPLETE (2026-06-09)](../../03-selling-and-shops/agent-native-setup/README.md) — 03 Selling & Shops · 7/7 stories · risk: High
- [Bulk Import & Express Migration](../../03-selling-and-shops/bulk-import-migration/README.md) — 03 Selling & Shops · 4 sprints · risk: High
- [Configurable & Personalized Products](../../03-selling-and-shops/configurable-personalized-products/README.md) — 03 Selling & Shops · 3 sprints · risk: High
- [Gem → Claimable Shop Loop (Ask-Claude / Mexico 26)](../../03-selling-and-shops/gem-claim-loop/README.md) — 03 Selling & Shops · 0/9 stories
- [Seller & unclaimed-shop bug sweep — ✅ COMPLETE 2026-06-10](../../03-selling-and-shops/seller-unclaimed-bug-sweep/README.md) — 03 Selling & Shops · 7/7 stories · risk: High
- [Seller Agent Operations](../../03-selling-and-shops/seller-agent-operations/README.md) — 03 Selling & Shops · 3 sprints
- [Seller Coupon Codes](../../03-selling-and-shops/promotions/README.md) — 03 Selling & Shops · 1 sprints
- [Shop Settings refactor](../../03-selling-and-shops/shop-settings-refactor/README.md) — 03 Selling & Shops · 21/21 stories · risk: Low
- [Printed-Edition Builder ("Maqueta")](../../06-print-edition/printed-edition-builder/README.md) — 06 Print · 0/8 stories
- [Agent Connection & Discoverability](../../07-agentic-and-federated-commerce/agent-connection/README.md) — 07 Agentic/Federated · 3 sprints · risk: High
- [Agent-readable why-sell / about surface](../../07-agentic-and-federated-commerce/agent-readable-about-surface/README.md) — 07 Agentic/Federated · 6/6 stories · risk: Low
- [Checkout on the custom domain (custom-domain-checkout)](../../07-agentic-and-federated-commerce/custom-domain-checkout/README.md) — 07 Agentic/Federated · 3 sprints · risk: High
- [Custom-domain paywall + campaign coupon](../../07-agentic-and-federated-commerce/custom-domain-paywall/README.md) — 07 Agentic/Federated · 0/9 stories · risk: High
- [Embeddable Widget](../../07-agentic-and-federated-commerce/embeddable-widget/README.md) — 07 Agentic/Federated · 3 sprints
- [Multi-tenant subdomains (`yourshop.miyagisanchez.com`)](../../07-agentic-and-federated-commerce/subdomains/README.md) — 07 Agentic/Federated · 1 sprints
- [Own channel: custom-domain polish (custom-domain-polish)](../../07-agentic-and-federated-commerce/custom-domain-polish/README.md) — 07 Agentic/Federated · 1 sprints · risk: Low
- [Own channel: full shop experience (own-shop-experience)](../../07-agentic-and-federated-commerce/own-shop-experience/README.md) — 07 Agentic/Federated · 3 sprints · risk: Low · Wave 3
- [Pretty URL: customizable shop slug (custom-slugs)](../../07-agentic-and-federated-commerce/custom-slugs/README.md) — 07 Agentic/Federated · 1 sprints
- [Short links (`mschz.org/shop` and `mschz.org/product`)](../../07-agentic-and-federated-commerce/short-links/README.md) — 07 Agentic/Federated · 1 sprints
- [Support Widget](../../07-agentic-and-federated-commerce/support-widget/README.md) — 07 Agentic/Federated · 4 sprints
- [Marketplace positioning — title, OG & social card](../../08-growth-and-promotions/marketplace-positioning-meta/README.md) — 08 Growth · 0/3 stories · risk: Low
- [Referral Program](../../08-growth-and-promotions/referral-program/README.md) — 08 Growth · 1 sprints · risk: Low
- [Seasonal Theme Engine](../../08-growth-and-promotions/seasonal-theme-engine/README.md) — 08 Growth · 3 sprints
- [Seller-Acquisition Landing Pages (BUILD-ORDER #6)](../../08-growth-and-promotions/seller-acquisition-landing-pages/README.md) — 08 Growth · 4/8 stories · risk: Low · Wave 3
- [Sweepstakes](../../08-growth-and-promotions/sweepstakes/README.md) — 08 Growth · 3 sprints · risk: Low
- [Backend Production Readiness — audit + hardening](../../09-platform-infra/backend-production-readiness/README.md) — 09 Platform-infra · 0/6 stories · risk: High
- [Cross-agent code review (second-opinion command)](../../09-platform-infra/cross-agent-code-review/README.md) — 09 Platform-infra · 0/4 stories · risk: Low
- [Cross-agent planning panel (single-pass advisory second opinions on plans)](../../09-platform-infra/cross-agent-planning-panel/README.md) — 09 Platform-infra · 4/4 stories · risk: Low
- [Design-Token / Design-System Foundation](../../09-platform-infra/design-token-foundation/README.md) — 09 Platform-infra · 3 sprints · risk: Low · Wave 0 Enablers
- [Feature flags & kill-switches (Flagsmith)](../../09-platform-infra/feature-flags-killswitches/README.md) — 09 Platform-infra · 2 sprints · risk: Low · Wave 0 Enablers
- [Navigation & Settings Reorg](../../09-platform-infra/navigation-settings-reorg/README.md) — 09 Platform-infra · 3/9 stories · risk: High
- [Unified CI/CD + Git event notifications via Telegram](../../09-platform-infra/cicd-telegram-notifications/README.md) — 09 Platform-infra · 3 sprints · risk: Low · Wave 0 Enablers
- [Events & Ticketing](../../10-events-and-ticketing/events-and-ticketing/README.md) — 10 Events & Ticketing · 0/7 stories · risk: High · Wave 4

## ⬜ Funnel — seeds not yet scaffolded (7)

- [Spike — arranged-only delivery policy (#3c Spike 0)](seeds/spike-arranged-only-delivery.md) — Ready · Spike · Wave 3
- [Buy-me-a-coffee tip widget](seeds/buy-me-a-coffee-widget.md) — Raw · Feature
- [Designer collaboration portal (deferred from #4)](seeds/designer-collaboration-portal.md) — Raw · Feature
- [Kill-switch flag decided at grooming, not at epic close](seeds/kill-switch-at-grooming.md) — Ready · Chore
- [Repo hygiene — branch cleanup + build-order/status reconciliation](seeds/repo-hygiene-and-build-order.md) — Ready · Chore
- [Spike — Compra protegida (buyer protection)](seeds/spike-compra-protegida.md) — Ready · Spike
- [URL / slug bug report](seeds/url-stuff.md) — Raw · Chore

## ⚠️ Status drift — frontmatter vs prose-derived (5)

These epics’ seed-frontmatter status disagrees with what the extractor derives from the sprint
docs. The board trusts **frontmatter**; the mismatch means one side is stale. Reconcile against
live code (see the cleanup handoff), then both this board and the Notion projection go correct.

| Epic | frontmatter (used) | prose-derived |
|---|---|---|
| Delivery & Manual-Money Polish | scaffolded | Shipped |
| Buyer Telegram channel + Buyer preference center | in-progress | Shipped |
| Granular Multi-Channel Notifications (Email + Telegram) | in-progress | Shipped |
| Trust & Messaging Polish | scaffolded | Shipped |
| Cross-channel Storefront Trust Parity | scaffolded | Shipped |

---
_Epics: 45 · seeds in funnel: 7 · status drift: 5. Regenerate with `node scripts/build-order.mjs`._
