...
| [neon-egress-and-db-isolation](neon-egress-and-db-isolation/) | Execute the approved Neon-egress spike: cache storefront reads, quiet the always-on backend (`minScale` + job cadence), split side-projects off the commerce Neon org, install site-wide Clarity, housekeeping | 🏗️ **Scaffolded 2026-06-21** (5 sprints) — root cause validated = `minScale:1` backend reading Neon cross-cloud ~190 MB/day (not backup, not traffic); keep commerce on Neon. S2/S3 HIGH (Daniel-merge/execute); S1/S4/S5 LOW–MED |
