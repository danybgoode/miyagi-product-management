---
status: shipped   # Both sprints merged 2026-07-10. S1: PR #208 squash 3bb5fb7 (StatusBadge/Button/Card + Toast/Banner, 23 tests). S2: PR #211 squash 37182fa (adoption sweep across 25 files -- the 4 named + OfferInbox/OrderDetail + PrintEditionCard + the full settings _sections/_components tree, ~2x the named scope, confirmed with Daniel -- + 4 new CI-lint checks in lib/design-token-audit.ts, gated on the swept file set only). Deterministic gate green both sprints (tsc+build+Playwright, 1819 passed on S2). Cross-agent review skipped on PR #211 (agy: diff too large for its argv-only input; codex: over quota) -- merged on green CI with Daniel's go-ahead. S2.5 follow-up (same session, PR #213 squash 6de222f): swept the two deferred files (CatalogTable.tsx + Envios.tsx, both PR #209/#210 having since merged), fixed a real gap in the lint's radius regex (missed directional/corner classes like rounded-l/rounded-r), found+fixed a real duplicate-ToggleSwitch bug in Envios.tsx, and agy cross-review (worked this time, smaller diff) caught one real bug pre-merge (title on a disabled button never shows -- moved to the wrapping label). Both files now in enforcedSweptPaths. Owed: Daniel's live visual smoke (light/dark/calm + undo path); the ~50 other seller-portal files never in this epic's scope still carry the same raw-palette/bg-white/literal-radii debt (visible via the lint's broad scan, not yet gated).
slug: seller-portal-rails-foundation
title: "Seller-portal rails foundation — one design language"
