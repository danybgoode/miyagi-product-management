...
| Sprint | Story | Risk |
|---|---|---|
| [S1](sprint-1.md) | B-1 · `standup-post` skill + `scripts/standup.mjs` → nightly Telegram standup (delta-only, `config.json` chat id, `standups.log` memory) | Low |
| [S1](sprint-1.md) | B-2 · Nightly "ops" routine that triggers the standup (one routine, not many — cap-safe) | Low |
...
| [S2](sprint-2.md) | B-6 · Fold B-3/B-4/B-5 outputs into the standup + the one nightly ops routine | Low |
