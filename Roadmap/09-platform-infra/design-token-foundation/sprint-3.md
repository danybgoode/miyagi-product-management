...
- [ ] A contrast check runs over the documented token pairs (`--fg`/`--bg`, `--fg-muted`/`--bg`,
      `--accent-foreground`/`--accent`, feedback-on-soft, etc.).
- [ ] All pairs pass AA (4.5:1 body / 3:1 large); any failure is fixed in `globals.css` or flagged in
      the epic with rationale.
- [ ] The check is reproducible (script or spec), not a one-off manual eyeball.
...
- [ ] A lightweight guard fails on a newly-introduced raw color in the target dirs — either an ESLint
      rule or a pure-logic Playwright `api` spec scanning the customer-facing component dirs.
- [ ] The guard **allow-lists** the legitimately-hardcoded contexts (email, print/PDF, OG image, admin,
      sandbox) so it doesn't false-positive on them.
- [ ] Pure logic lives on a next-free `lib/` seam so the `api` runner can load it (LEARNINGS → Tooling).
- [ ] The guard runs in the deterministic gate (green required before merge).

## Sprint 3 QA

- [ ] Contrast check passes AA for all documented pairs (output captured in the epic/retro).
- [ ] New-guard spec is in the `api` project and green; a deliberate raw-hex in a guarded dir makes it red
      (negative test demonstrated).
- [ ] `tsc --noEmit` + `npm run build` + Playwright `api` suite green.
