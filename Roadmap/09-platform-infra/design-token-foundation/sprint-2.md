...
- [ ] The **32** Tailwind arbitrary color classes (`bg-[#…]`, `text-[#…]`, etc.) in customer-facing
      components are swapped to token-backed classes/vars.
- [ ] Prioritized raw-hex literals in live surfaces are tokenized: `CheckoutExperience.tsx`,
      `embed/s/[slug]/page.tsx`, `ConversationClient.tsx`, `ShopSettings.tsx`, `ImportClient.tsx`,
      `ClaimForm.tsx`, and the small-offender tail.
- [ ] **Explicitly excluded (documented why):** `lib/email.ts` (email clients strip CSS vars),
...
- [ ] **Zero visible change:** the rendered output of every touched page is pixel-identical before/after.

## Sprint 2 QA

- [ ] Deterministic gate green: `tsc --noEmit` + `npm run build` + Playwright `api` suite.
- [ ] **Anonymous browser smoke**: load key pages (home, a listing, checkout entry, an embed) on the
      branch preview and confirm no visual regression vs `main`.
- [ ] **Owed to Daniel:** a before/after **screenshot diff** on those key pages (the sampled smoke can't
      fully prove "zero visible change" — a human eye on the diff closes it).
