...
**Status:** ✅ merged 2026-07-05 — [PR #176](https://github.com/danybgoode/miyagisanchezcommerce/pull/176) (`d6d457b`), frontend-only, 3 sub-stories, Daniel-authorized merge on green CI. Cross-reviewed (Antigravity, 2 rounds): round 1 caught a real blocking bug (a paused converted listing 422'd EVERY save because the form always sent the flat price — fixed with a parsed-value dirty check) + stale mirror price on tier edits + a grid prop state-trap; round 2's real find (paused multi-variant listings still *rendering* the flat inputs) fixed by stamping `has_variants:true` on the mirror at convert; its Rule-1/MCP-manifest/read-race findings dismissed as false positives with citations (see PR comments + commit `3db8ce6`). CI green on all pushes.
...
Env: everything is merged — run against prod, `https://miyagisanchez.com` (Story 2.4's UI deployed with frontend `d6d457b`; backend unchanged since `d22fb29`).
